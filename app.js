...
// Workshop-closed notice on page load, dismissed for the rest of the session
const closedModal = document.getElementById("closed-modal");

if (closedModal) {
  const closedModalClose = document.getElementById("closed-modal-close");
  const STORAGE_KEY = "csi_closed_notice_dismissed";
  let lastFocused = null;

  // Private windows and blocked site data make storage throw, so never let it
  // decide whether the page works.
  const wasDismissed = () => {
    try {
  ...
  const rememberDismissal = () => {
    try {
      ...
      /* nothing to do — the notice just reappears next load */
    }
  };
  ...
  const showModal = () => {
    ...
    // Force a reflow so the browser registers the pre-transition state before
    // .active flips it; rAF is unreliable in headless/background tabs.
    void closedModal.offsetWidth;
    closedModal.classList.add("active");
    if (closedModalClose) closedModalClose.focus();
  };

  const hideModal = () => {
    closedModal.classList.remove("active");
    rememberDismissal();
    // must outlast the longest close transition (.38s backdrop) or the fade
    // gets cut off by display:none
    setTimeout(() => {
  ...
  if (closedModalClose) closedModalClose.addEventListener("click", hideModal);

  // Click the dimmed area outside the card to dismiss
  closedModal.addEventListener("click", (e) => {
    if (e.target === closedModal) hideModal();
  });

  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && !closedModal.hidden) hideModal();
  });
}
